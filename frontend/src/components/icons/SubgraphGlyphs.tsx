/**
 * Subgraph glyphs. Outlines taken from the reference: brackets mark a subgraph, arrows
 * outward and inward mean "expand" and "collapse". Kept separate from the buttons: they need
 * their own size in each place.
 */
...
export function SubgraphGlyph({ className = "size-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className}>
      <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={SUBGRAPH_PATH} />
    </svg>
  );
}
